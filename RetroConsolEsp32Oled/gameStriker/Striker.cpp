...
void Game_Striker() {
  ...
  // Striker.sesionScore = 0;
  while (true) {
    WelcomeStrikerScreen();
  //   StrikerInit(Striker, allBallLines); 
  //   WaitForAnyButtonToContinue();
  //   Striker.prevUpdateTime  = millis();

  //   while (Striker.lives!=0) {
  //     DisplaySoundInfo(120, 0, SoundEnabled);
  //     if (ItsTimeToMoveBall(Striker)) {
  //       DisplayBallsOnScreen(allBallLines);
  //       CheckIfBallDropped(Striker, allBallLines);
  //       // update game speed every 10 points
  //       if (Score > 30) Striker.frameUpdateRate = Striker.initFrameUpdateRate - (Score/10)*15;
  //       if (Striker.lives == 0) break;
  //       if (NewBallIsRequired(Striker, allBallLines)) {
  //         for (int i = 0; i < 4; i++) {
  //           DrawBalls(allBallLines[i], 1);
  //         }
  //       }
  //     }
  //     StrikerDisplayScore(Striker);
  //     CheckIfSnopyMoved(Striker);
  //     myOLED.display();
  //   }
  // }
  }
...
//   orientation prevPos = Striker.direction;

//   if (IsPressed(UpLeft)) Striker.direction = sLeftUp;
//   if (IsPressed(DownLeft)) Striker.direction = sLeftDown;
//   if (IsPressed(UpRight)) Striker.direction = sRightUp; 
//   if (IsPressed(DownRight)) Striker.direction = sRightDown;
  
//   DrawStriker(prevPos, SH110X_BLACK);
//   DrawStriker(Striker.direction, SH110X_WHITE);
// }

// void DrawStriker(orientation Pos, uint16_t color) {
//     switch(Pos) {
//     case sLeftDown:
//       myOLED.drawBitmap(30, 13, StrikerLD, 50, 50, color);
//       break;
//     case sLeftUp:
//       myOLED.drawBitmap(31, 10, StrikerLU, 50, 50, color);
//       break;
//     case sRightDown:
//       myOLED.drawBitmap(45, 13, StrikerRD, 50, 50, color);
//       break;
//     case sRightUp:
//       myOLED.drawBitmap(44, 10, StrikerRU, 50, 50, color);
//       break;
//     case sWaiting:
//       myOLED.drawBitmap(37, 10, StrikerSit, 50, 50, color);
//       break;
//     case sResting:
//       myOLED.drawBitmap(37, 10, StrikerNap, 50, 50, color);
//       break;
//   }
// }
...
//   if (millis()- Striker.prevUpdateTime > (Striker.frameUpdateRate)) {
//     Striker.prevUpdateTime  = millis();
//     return true;
//   }
//   return false;
...
//   Striker.lives = 3;
//   Score = 0;
//   Striker.direction = sResting;
//   Striker.frameUpdateRate = Striker.initFrameUpdateRate;
//   StrikerDisplayScore(Striker);
//   DrawStriker(Striker.direction, SH110X_WHITE);
...
//       if (Striker.direction == allBallLines[i].direction) {
//         Score++;
//         if(SoundEnabled) {
...
//       } else {
//         Striker.lives --;
//         DisplayBrokenBall(Striker, allBallLines[i].direction);

//         if (Striker.lives == 0) {
//           DisplayGameOverStriker(Striker);
//           WaitForAnyButtonToContinue();
//         }
//         ClearBallLines(allBallLines);
//       }
...
//   DrawStriker(Striker.direction, SH110X_BLACK);
//   Striker.direction = sWaiting;
//   DrawStriker(Striker.direction, SH110X_WHITE);
...
//   myOLED.drawCircle(2 + Striker.lives * 8, 58, 2, SH110X_BLACK);
// }
...
//   if (Score > Striker.sesionScore) {
//     Striker.sesionScore = Score;
//   }
  
//   // Sprawdź i zapisz highscore
//   int highscore;
//   EEPROM.get(Game_StrikerRecord, highscore);
...
//   if (Score > highscore) {
//     EEPROM.put(Game_StrikerRecord, Score);
//     EEPROM.commit();
...
//   myOLED.print(Striker.sesionScore);
